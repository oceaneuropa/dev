...
public class NettyContainerTest {
	...
	public void testChunkedOutput() throws IOException, URISyntaxException, ExecutionException, InterruptedException {
		...
		ListenableFuture<Object> response = client.prepareGet("http://localhost:" + port + "/").execute(new AsyncHandler<Object>() {

			private List<String> chunks = new ImmutableList.Builder<String>().add(repeat("a", 8192)).add(repeat("b", 8192)).build();
	...
	private ServerBootstrap getServerBootstrap() {
		final ExecutorService bossExecutor = Executors.newCachedThreadPool(new ThreadFactoryBuilder().setNameFormat("restapi-boss-%d").build());

		final ExecutorService workerExecutor = Executors.newCachedThreadPool(new ThreadFactoryBuilder().setNameFormat("restapi-worker-%d").build());
...
//	}
	...
	private Resource getResource(Inflector<ContainerRequestContext, ChunkedOutput<?>> inflector) {
		...
		methodBuilder.produces(MediaType.TEXT_PLAIN_TYPE).handledBy(inflector);
		return resourceBuilder.build();
	}

//	private NettyContainer getNettyContainer(Resource resource, Class... classes) throws URISyntaxException {
//		ResourceConfig rc = new ResourceConfig().property(NettyContainer.PROPERTY_BASE_URI, new URI("http:/localhost:0")).registerResources(resource).registerInstances(new NettyContainerProvider()).register(JacksonJsonProvider.class);
...
//	private int bindJerseyServer(Inflector<ContainerRequestContext, ChunkedOutput<?>> inflector, ServerBootstrap bootstrap, Class... classes) throws URISyntaxException {
