 ...
 * 
 * @see https://www.programcreek.com/java-api-examples/index.php?source_dir=jersey-netty-master/src/main/java/org/graylog2/jersey/container/netty/NettyContainer
 *      .java
 */
public class NettyContainer extends SimpleChannelUpstreamHandler implements Container {
		...
		public void commit() {
			if (channel.isOpen()) {
				...
				if (httpResponse.isChunked()) {
					...
					if (log.isTraceEnabled()) {
						log.trace("Writing entire {} bytes to client {}", httpResponse.getContent().readableBytes(), channel.getRemoteAddress());
					}
	...
	public void messageReceived(ChannelHandlerContext ctx, MessageEvent e) throws Exception {
		...
		String[] schemeCreds = extractBasicAuthCredentials(HttpHeaders.getHeader(httpRequest, HttpHeaders.Names.AUTHORIZATION));
		String scheme = null;
		String user = null;
		...
		final SecurityContext securityContext = securityContextFactory.create(user, password, isSecure, scheme, ctx.getChannel().getRemoteAddress().toString());
		ContainerRequest containerRequest = new ContainerRequest(baseUri, requestUri, httpRequest.getMethod().getName(), securityContext, new MapPropertiesDelegate());
...
//		}
		...
		containerRequest.setWriter(new NettyResponseWriter(protocolVersion, closeConnection, e.getChannel()));
...
//		}
	...
	public void exceptionCaught(ChannelHandlerContext ctx, ExceptionEvent e) throws Exception {
		...
		if ((protocolVersion == HttpVersion.HTTP_1_0) || request == null || HttpHeaders.getHeader(request, HttpHeaders.Names.CONNECTION).equalsIgnoreCase("close")) {
			channelFuture.addListener(ChannelFutureListener.CLOSE);
		}
	}
