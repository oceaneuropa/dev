...
/**
 * A filter that keeps authenticated sessions alive by extending the cookie expiration time. It is crucial that this filter is executed AFTER the
...
public class SessionKeepAliveFilter implements ResourceFilter, ContainerResponseFilter {
	...
	public ContainerResponse filter(ContainerRequest request, ContainerResponse response) {
		...
		if (ticketCookies != null && ticketCookies.size() > 0) {
			// If there is a valid ticket cookie in the request, extend its expiration time. Invalid ticket cookes are removed from the request by the
			// Authentication filter which should have already run
