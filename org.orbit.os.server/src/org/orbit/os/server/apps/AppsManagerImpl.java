...
public class AppsManagerImpl implements AppsManager {
	...
	protected static Logger LOG = LoggerFactory.getLogger(AppsManagerImpl.class);
	...
	protected void activateAppHandler(AppHandler appHandler) throws AppException {
		LOG.info("activateAppHandler() appHandler = " + appHandler);
	...
	protected void deactivateAppHandlers() throws AppException {
		LOG.info("deactivateAppHandlers()");
	...
	protected void deactivateAppHandler(AppHandler appHandler) throws AppException {
		LOG.info("deactivateAppHandler() appHandler = " + appHandler);
	...
	protected synchronized AppHandler registerApp(AppManifest appManifest) throws AppException {
		LOG.info("registerApp() appManifest = " + appManifest);
		...
		AppHandler appHandler = new AppHandlerImpl(appManifest, this.bundleContext);
		activateAppHandler(appHandler);
	...
	protected synchronized boolean unregisterApp(AppManifest appManifest) throws AppException {
		LOG.info("unregisterApp() appManifest = " + appManifest.getSimpleName());
	...
	public AppManifest install(Path appArchivePath) throws AppException {
		LOG.info("install() appArchivePath = " + appArchivePath.toString());

		checkStarted();

		AppManifest appManifestFromArchive = AppUtil.extractAppManifest(appArchivePath.toFile());
		if (appManifestFromArchive == null) {
			LOG.info("App manifest cannot be found in the app archive file '" + appArchivePath.toString() + "'.");
			return null;
		}
