...
import org.orbit.component.webconsole.servlet.origin.CreateNewOSPage;
import org.orbit.component.webconsole.servlet.origin.CreateNewOSServlet;
...
public class OriginWebApplication extends PlatformWebApplication {
	...
	protected void addResources(BundleContext bundleContext) {
		...
		addServlet(new ServletMetadataImpl(WebConstants.ORIGIN_MESSAGE_PAGE_PATH, new MessagePage(), dicts));
		addServlet(new ServletMetadataImpl(WebConstants.ORIGIN_LOGIN_SERVLET_PATH, new LoginServlet(), dicts));
		addServlet(new ServletMetadataImpl(WebConstants.ORIGIN_LOGOUT_SERVLET_PATH, new LogoutServlet(), dicts));
		addServlet(new ServletMetadataImpl(WebConstants.ORIGIN_CREATE_NEW_ACCOUNT_PAGE_PATH, new CreateNewAccountPage(), dicts));
		addServlet(new ServletMetadataImpl(WebConstants.ORIGIN_CREATE_NEW_ACCOUNT_SERVLET_PATH, new CreateNewAccountServlet(), dicts));
