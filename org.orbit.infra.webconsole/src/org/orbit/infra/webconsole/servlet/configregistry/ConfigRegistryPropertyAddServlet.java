...
public class ConfigRegistryPropertyAddServlet extends HttpServlet {

	private static final long serialVersionUID = 6779041150688008242L;
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (!configRegistryId.isEmpty() && !name.isEmpty()) {
			try {
				...
				if (!cfg.isOnline()) {
					...
					if (configReg == null) {
						...
						succeed = configReg.setProperty(null, name, value);
					}
				}
			} catch (Exception e) {
			...
			if (succeed) {
				message = MessageHelper.INSTANCE.add(message, "Property is added.");
			} else {
				message = MessageHelper.INSTANCE.add(message, "Property is not added.");
			}
		}
