...
public class AppStoreWSAppResource extends AbstractWSApplicationResource {
	...
	@Path("/exists")
	@Produces(MediaType.APPLICATION_JSON)
	public Response appExists(@QueryParam("appId") String appId, @QueryParam("appVersion") String appVersion) {
	...
	public Response downloadApp(@QueryParam("appId") String appId, @QueryParam("appVersion") String appVersion) {
		if (appId == null || appId.isEmpty()) {
			ErrorDTO nullAppIdError = new ErrorDTO("appId is null.");
			return Response.status(Status.BAD_REQUEST).entity(nullAppIdError).build();
		}
