...
public class AppStoreWSAppResource extends AbstractWSApplicationResource {
	...
	@GET
	@Produces(MediaType.APPLICATION_JSON)
	public Response get(@QueryParam("appId") String appId, @QueryParam("appVersion") String appVersion) {
		...
		try {
			...
			appDTO = RuntimeModelConverter.AppStore.toAppDTO(app);
...
// URL (POST): {scheme}://{host}:{port}/{contextRoot}/app/upload?id={id}&appId={appId}&appVersion={appVersion} (FormData: InputStream and
// FormDataContentDisposition) // Upload an app.
