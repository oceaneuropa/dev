...
public class UserRegistryServiceAdapter implements LifecycleAware {
	...
	protected void doStart(BundleContext bundleContext, UserRegistryService service) {
		...
		IExtension extension = PlatformSDKActivator.getInstance().getExtensionRegistry().getExtension(ServiceIndexTimerFactory.EXTENSION_TYPE_ID, ComponentsConstants.USER_REGISTRY_INDEXER_ID);
		if (extension != null) {
			// String indexProviderId = extension.getId();
			@SuppressWarnings("unchecked")
			ServiceIndexTimerFactory<UserRegistryService> indexTimerFactory = extension.createExecutableInstance(ServiceIndexTimerFactory.class);
