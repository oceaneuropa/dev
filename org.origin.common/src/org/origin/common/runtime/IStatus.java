...
public interface IStatus {
	...
	/**
	 * Status severity constant (value 0) indicating this status represents the nominal case. This constant is also used as the status code
	...
	/**
	 * Returns the relevant low-level exception, or <code>null</code> if none. For example, when an operation fails because of a network
	 * communications failure, this might return the <code>java.io.IOException</code> describing the exact nature of that failure.
	 *
	 ...
	 * </ul>
	 * <p>
	 * The severity of a multi-status is defined to be the maximum severity of any of its children, or <code>OK</code> if it has no children.
	 * </p>
	 *
	 * @return the severity: one of <code>OK</code>, <code>ERROR</code>, <code>INFO</code>, <code>WARNING</code>, or <code>CANCEL</code>
	 * @see #matches(int)
	 */
	 ...
	 * <p>
	 * The severity of a multi-status is derived from the severities of its children; a multi-status with no children is <code>OK</code> by
	 * definition. A multi-status carries a plug-in identifier, a status code, a message, and an optional exception. Clients may treat multi-status
	...
	/**
	 * Returns whether the severity of this status matches the given severity mask. Note that a status with severity <code>OK</code> will never match;
	 * use <code>isOK</code> instead to detect a status with a severity of <code>OK</code>.
	 *
	 * @param severityMask
	 *            a mask formed by bitwise or'ing severity mask constants (<code>ERROR</code>, <code>WARNING</code>, <code>INFO</code>,
	 *            <code>CANCEL</code>)
