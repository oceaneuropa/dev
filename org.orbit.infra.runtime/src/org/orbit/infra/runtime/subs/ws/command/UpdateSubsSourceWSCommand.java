...
public class UpdateSubsSourceWSCommand extends AbstractInfraCommand<SubsServerService> implements WSCommand {
	...
	public Response execute(Request request) throws Exception {
		...
		if (!hasTypeParams && !hasNameParam && !hasPropertiesParam) {
			ErrorDTO error = new ErrorDTO("('type' and 'typeId') parameters or 'name' parameter or 'properties' parameter are not set.");
			return Response.status(Status.BAD_REQUEST).entity(error).build();
		}
		...
		if (hasTypeParams) {
			...
			boolean currSucceed = service.updateSourceType(id, type, typeId);
			if (currSucceed) {
				hasSucceed = true;
			} else {
				hasFailed = true;
