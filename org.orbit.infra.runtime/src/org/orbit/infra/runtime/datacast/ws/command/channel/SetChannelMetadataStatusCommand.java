...
public class SetChannelMetadataStatusCommand extends AbstractInfraCommand<DataCastService> implements WSCommand {

	public static String ID = "org.orbit.infra.runtime.datacast.SetChannelMetadataStatusCommand";

	public SetChannelMetadataStatusCommand() {
	...
	public boolean isSupported(Request request) {
		...
		if (RequestConstants.DATACAST__SET_CHANNEL_METADATA_STATUS.equalsIgnoreCase(requestName)) {
			return true;
		}
		return false;
	}
	...
	public Response execute(Request request) throws Exception {
		...
		boolean append = request.getBooleanParameter("append");
		...
		if (channelMetadata != null) {
			if (append) {
				channelMetadata.appendStatus(status);
			} else {
				channelMetadata.setStatus(status);
			}
