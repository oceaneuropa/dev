...
public class FileSystemServiceHelper {
	...
	public static FileSystemServiceHelper INSTANCE = new FileSystemServiceHelper();
	 ...
	 * @throws IOException
	 */
	public boolean copyFsDirectoryToLocalDirectory(FileSystemService fs, FilePath sourceDirPath, File localDir, boolean includingSourceDir) throws IOException {
		...
		if (includingSourceDir) {
			...
			FilePath[] memberPaths = fs.listFiles(sourceDirPath);
	 ...
	 * @throws IOException
	 */
	public void copyFsFileToLocalDirectory(FileSystemService fs, FilePath sourcePath, File localDir) throws IOException {
		...
		if (metadata.isDirectory()) {
			...
			FilePath[] memberPaths = fs.listFiles(sourcePath);
			for (FilePath memberPath : memberPaths) {
	 ...
	 * @param level
	 */
	public void walkFolders(FileSystemService fs, FilePath path, int level) {
		...
		if (fileMetaData.isDirectory()) {
			...
			FilePath[] subPaths = fs.listFiles(path);
