...
public class DatabaseFS extends FileSystemServiceImpl {

	/**
	 * 
	 * @param config
	 */
	public DatabaseFS(DatabaseFSConfig config) {
		setConfig(config);
	}

	protected Connection getConnection() {
		Connection conn = null;
		if (getConfig() instanceof DatabaseFSConfig) {
			conn = ((DatabaseFSConfig) getConfig()).getConnection();
		}
		return conn;
	}

	public static FsFileMetadataTableHandler getFileMetadataHandler() {
		return DatabaseFSUtil.getFileMetadataHandler();
	}

	public static FsFileContentTableHandler getFileContentHandler() {
		return DatabaseFSUtil.getFileContentHandler();
	}
	...
	public FileMetadata getFileMetaData(FilePath path) {
		...
		try {
			...
			if (segments == null || segments.length == 0) {
				...
				return DatabaseFSUtil.createNonExistsFileMetadata(path);
			}
			...
			for (int i = 0; i < segments.length; i++) {
				...
				if (!isLastSegment) {
					...
					return DatabaseFSUtil.createFileMetadata(path, vo);
				}
			}
	...
	public FilePath[] listRoots() {
		...
		try {
			...
			if (rootVOs != null) {
				FilePath[] paths = new FilePath[rootVOs.size()];
		...
		try {
			...
			for (int i = 0; i < segments.length; i++) {
				...
				if (!isLastSegment) {
					...
					if (memberVOs != null) {
						FilePath[] paths = new FilePath[memberVOs.size()];
		...
		try {
			...
			for (int i = 0; i < segments.length; i++) {
				...
				if (!isLastSegment) {
					...
					return DatabaseFSUtil.delete(conn, vo.getFileId());
				}
			}
		} catch (SQLException e) {
			e.printStackTrace();
		...
		try {
			...
			for (int i = 0; i < segments.length; i++) {
				...
				if (!isLastSegment) {
					...
					// using Postgres database
					DatabaseFSUtil.writeFileContentPostgres(conn, vo.getFileId(), inputStream);
				}
			}
		} catch (SQLException e) {
			e.printStackTrace();
			...
			for (int i = 0; i < segments.length; i++) {
				...
				if (!isLastSegment) {
					...
					try {
						...
						DatabaseFSUtil.writeFileContentPostgres(conn, vo.getFileId(), fis);
					} finally {
	 ...
	 * @throws IOException
	 */
	protected void doCopyLocalFileOrDirectoryToFsDirectory(File localFile, FilePath destDirPath, List<File> encounteredFiles) throws IOException {
