package org.nb.mgm.util;

public class SearchPattern {

	// Rules for pattern matching: (exact, prefix, pattern) [ | case sensitive]
	/**
	 * Match rule: The search pattern matches exactly the search result, that is, the source of the search result equals the search pattern. Search
	...
	/**
	 * Match rule: The search pattern contains one or more wild cards ('*' or '?'). A '*' wild-card can replace 0 or more characters in the search
	 * result. A '?' wild-card replaces exactly 1 character in the search result.
	 */
	...
	public static SearchPattern createPattern(String text) {
	...
	/**
	 * Creates new instance of SearchPattern Default allowedRules for it is result of belong logic operation: ( RULE_EXACT_MATCH | RULE_PREFIX_MATCH |
	...
	/**
	 * Creates a search pattern with the rule to apply for matching index keys. It can be exact match, prefix match, pattern match or camelCase match.
	 * Rule can also be combined with a case sensitivity flag.
	 * 
	 * @param allowedRules
	 *            one of {@link #RULE_EXACT_MATCH}, {@link #RULE_PREFIX_MATCH}, {@link #RULE_PATTERN_MATCH}, {@link #RULE_CASE_SENSITIVE},
	 *            {@link #RULE_CAMELCASE_MATCH} combined with one of following values: {@link #RULE_EXACT_MATCH}, {@link #RULE_PREFIX_MATCH},
	 *            {@link #RULE_PATTERN_MATCH} or {@link #RULE_CAMELCASE_MATCH}. e.g. {@link #RULE_EXACT_MATCH} | {@link #RULE_CASE_SENSITIVE} if an
	 *            exact and case sensitive match is requested, {@link #RULE_PREFIX_MATCH} if a prefix non case sensitive match is requested or
	 *            {@link #RULE_EXACT_MATCH} if a non case sensitive and erasure match is requested.<br>
	 ...
	 * @param text
	 * @param prefix
	 * @return true if text starts with given prefix, ignoring case false in other way
	 */
	...
	/**
	 * Answers true if the pattern matches the given name using CamelCase rules, or false otherwise. CamelCase matching does NOT accept explicit
	 * wild-cards '*' and '?' and is inherently case sensitive. <br>
	 * CamelCase denotes the convention of writing compound names without spaces, and capitalizing every term. This function recognizes both upper and
	 * lower CamelCase, depending whether the leading character is capitalized or not. The leading part of an upper CamelCase pattern is assumed to
	 * contain a sequence of capitals which are appearing in the matching name; e.g. 'NPE' will match 'NullPointerException', but not 'NewPerfData'. A
	 * lower CamelCase pattern uses a lowercase first character. In Java, type names follow the upper CamelCase convention, whereas method or field
	 * names follow the lower CamelCase convention. <br>
	 * The pattern may contain lowercase characters, which will be match in a case sensitive way. These characters must appear in sequence in the
	 * name. For instance, 'NPExcep' will match 'NullPointerException', but not 'NullPointerExCEPTION' or 'NuPoEx' will match 'NullPointerException',
	 * but not 'NoPointerException'. <br>
	 * <br>
	 * Examples:
	 * <ol>
	 * <li>
	...
	/**
	 * Answers true if a sub-pattern matches the subpart of the given name using CamelCase rules, or false otherwise. CamelCase matching does NOT
	 * accept explicit wild-cards '*' and '?' and is inherently case sensitive. Can match only subset of name/pattern, considering end positions as
	 * non-inclusive. The subpattern is defined by the patternStart and patternEnd positions. <br>
	 * CamelCase denotes the convention of writing compound names without spaces, and capitalizing every term. This function recognizes both upper and
	 * lower CamelCase, depending whether the leading character is capitalized or not. The leading part of an upper CamelCase pattern is assumed to
	 * contain a sequence of capitals which are appearing in the matching name; e.g. 'NPE' will match 'NullPointerException', but not 'NewPerfData'. A
	 * lower CamelCase pattern uses a lowercase first character. In Java, type names follow the upper CamelCase convention, whereas method or field
	...
	/**
	 * Returns the rule to apply for matching keys. Can be exact match, prefix match, pattern match or camelcase match. Rule can also be combined with
	 * a case sensitivity flag.
	 * 
	 * @return one of RULE_EXACT_MATCH, RULE_PREFIX_MATCH, RULE_PATTERN_MATCH, RULE_CAMELCASE_MATCH, combined with RULE_CASE_SENSITIVE, e.g.
	 *         RULE_EXACT_MATCH | RULE_CASE_SENSITIVE if an exact and case sensitive match is requested, or RULE_PREFIX_MATCH if a prefix non case
	 ...
	 * <ul>
	 * <li>{@link #RULE_PATTERN_MATCH} without any '*' or '?' in string pattern: pattern match bit is unset,</li>
	 * <li>{@link #RULE_PATTERN_MATCH} and {@link #RULE_PREFIX_MATCH} bits simultaneously set: prefix match bit is unset,</li>
	 * <li>{@link #RULE_PATTERN_MATCH} and {@link #RULE_CAMELCASE_MATCH} bits simultaneously set: camel case match bit is unset,</li>
	 * <li>{@link #RULE_CAMELCASE_MATCH} with invalid combination of uppercase and lowercase characters: camel case match bit is unset and replaced
	 * with prefix match pattern,</li>
	 * <li>{@link #RULE_CAMELCASE_MATCH} combined with {@link #RULE_PREFIX_MATCH} and {@link #RULE_CASE_SENSITIVE} bits is reduced to only
	 * {@link #RULE_CAMELCASE_MATCH} as Camel Case search is already prefix and case sensitive,</li>
	 * </ul>
	 * <br>
	...
	/**
	 * Tells whether the given <code>SearchPattern</code> is a sub-pattern of this pattern.
	 * <p>
	 * <i>WARNING: This method is <b>not</b> defined in reading order, i.e. <code>a.isSubPattern(b)</code> is <code>true</code> iff <code>b</code> is
