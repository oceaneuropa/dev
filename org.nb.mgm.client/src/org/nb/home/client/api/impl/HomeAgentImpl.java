...
public class HomeAgentImpl implements HomeAgent {
	...
	@Override
	public synchronized void buildProject(String projectHomeId) throws ClientException {
		checkConnection();
	...
	public <T> T getAdapter(Class<T> adapter) {
		if (ManagementClient.class.isAssignableFrom(adapter)) {
			if (this.managementClient != null) {
				return (T) this.managementClient;
			}
		}
		if (ClientConfiguration.class.isAssignableFrom(adapter)) {
			return (T) this.clientConfig;
		}
		if (HomeAgentWSClient.class.isAssignableFrom(adapter)) {
