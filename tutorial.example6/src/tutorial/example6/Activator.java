 ...
 * 
 * http://felix.apache.org/documentation/tutorials-examples-and-presentations/apache-felix-osgi-tutorial/apache-felix-tutorial-example-6.html
 * 
 * 
 * This class implements a bundle that implements a spell checker service. The spell checker service uses all available dictionary services to check
 * for the existence of words in a given sentence. This bundle not only monitors the dynamic availability of dictionary services, but it manages the
 * aggregation of all available dictionary services as they arrive and depart. The spell checker service is only registered if there are dictionary
 * services available, thus the spell checker service will appear and disappear as dictionary services appear and disappear, respectively.
 **/
public class Activator implements BundleActivator, ServiceListener {
	...
	/**
	 * Implements BundleActivator.start(). Adds itself as a service listener and queries for all currently available dictionary services. Any
	 * available dictionary services are added to the service reference list. If dictionary services are found, then the spell checker service is
	 * registered.
	 * 
	 * @param context
		...
		synchronized (m_refList) {
			...
			if (refs != null) {
				for (int i = 0; i < refs.length; i++) {
					...
					// Make sure that the service is not being duplicated.
				...
				// dictionary services.
				if (m_refList.size() > 0) {
					m_reg = m_context.registerService(SpellChecker.class.getName(), new SpellCheckerImpl(), null);
				}
			}
		}
	}
	...
	/**
	 * Implements BundleActivator.stop(). Does nothing since the framework will automatically unregister any registered services, release any used
	...
	/**
	 * Implements ServiceListener.serviceChanged(). Monitors the arrival and departure of dictionary services, adding and removing them from the
	 * service reference list, respectively. In the case where no more dictionary services are available, the spell checker service is unregistered.
	 * As soon as any dictionary service becomes available, the spell checker service is reregistered.
	 * 
	 * @param event
	...
	/**
	 * A private inner class that implements a spell checker service; see SpellChecker for details of the service.
	 **/
