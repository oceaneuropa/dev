...
public class ResourcePropertyHelper {
	...
	public <S> boolean hasLastHeartBeatTime(LoadBalanceResource<S> resource) {
		if (resource.hasProperty(OriginConstants.LAST_HEARTBEAT_TIME)) {
			Object value = resource.getProperty(OriginConstants.LAST_HEARTBEAT_TIME);
	...
	public <S> boolean hasLastPingTime(LoadBalanceResource<S> resource) {
		if (resource.hasProperty(OriginConstants.LAST_PING_TIME)) {
			Object value = resource.getProperty(OriginConstants.LAST_PING_TIME);
			if (value instanceof Date) {
				return true;
	...
	public <S> boolean hasLastPingSucceed(LoadBalanceResource<S> resource) {
		if (resource.hasProperty(OriginConstants.LAST_PING_SUCCEED)) {
			Object value = resource.getProperty(OriginConstants.LAST_PING_SUCCEED);
